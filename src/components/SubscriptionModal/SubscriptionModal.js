...
class SubscriptionModal extends React.Component {
  ...
  componentDidMount() {
    const { stripeSubscriptionId, paypalSubscriptionId, userLimit } = this.props.subscriberOrg || {};
    ...
    if (paypalSubscriptionId) {
      this.props
        ...
        .then(() => {
          ...
          if (paypalSubscription.state !== 'Suspended') {
  ...
  handleSubscriptionPeriod = () => {
    const { subscription, paypalSubscription } = this.props;
    if (paypalSubscription && paypalSubscription.state === 'Active' && paypalSubscription.plan) {
      let paypalPeriodEnd;
      if (paypalSubscription.plan.payment_definitions[0].frequency === 'Month') {
        paypalPeriodEnd = moment(paypalSubscription.start_date).add(1, 'M');
      } else {
            ...
            <div className="Modal_header">
              ...
              <h5 className="Modal_title">
                <span className="habla-bold-text">
                  {subscription.cancel_at_period_end && paypalSubscription.state === 'Suspended'
