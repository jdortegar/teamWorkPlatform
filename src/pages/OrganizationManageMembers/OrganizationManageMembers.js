...
class OrganizationManageMembers extends Component {
  ...
  handleChangeStatus(checked, userId) {
    ...
    this.props
      ...
      .then(() => message.success(String.t('OrganizationManageMembers.userUpdated')))
  ...
  renderUsers(usersActive) {
    const { usersPresences, org, orgId } = this.props;
    ...
    userArray.unshift({
      ...
      user: {
          ...
          <div>
            ...
            <span className="MembersPage__membersLeft_badge">
              {String.t('OrganizationManageMembers.seatAvailables', {
                count: org.userLimit - this.state.usersActive.length || 0
              })}
            </span>
          </div>
        ),
        ...
        editUrl: `/app/inviteNewMember/${orgId}`,
        addButton: true
      },
      status: {
        enabled: false,
